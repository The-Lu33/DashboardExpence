...
export default function Add() {
  ...
  const onChangeTime = (e: DateTimePickerEvent, selectDate?: Date) => {
    ...
    const formattedDate = dayjs(currentDate).format("hh:mm A");
  ...
  const onChangeTimeAndroid = (e: DateTimePickerEvent, selectDate?: Date) => {
    ...
    if (selectDate) {
      const formateTime = dayjs(selectDate).format("hh:mm A");
          ...
          {Platform.OS === "ios" && showSelectDate && (
            ...
            >
              <View
                ...
                onTouchEnd={() => setShowSelectDate(false)} // This will close the modal when tapping outside
              >
                <TouchableWithoutFeedback>
                  <View
                    className="w-full h-[33%] rounded-t-xl pt-2 bg-white dark:bg-black/90"
                    onTouchEnd={(e) => e.stopPropagation()} // Prevent closing when tapping inside the modal
                  >
                    <View>
                      <View className="mx-auto mt-4">{renderDatePicker()}</View>
                      <TouchableHighlight
                        className="absolute left-2 border rounded-lg border-red-500 px-2 py-1"
                      ...
                      <TouchableHighlight
                        className="absolute right-2 px-2 py-1 bg-green-500 rounded-lg"
                        underlayColor={"transparent"}
                        onPress={() => {
                          setShowSelectDate(false);
                        }}
          ...
          {Platform.OS === "ios" && showSelectTime && (
            ...
            >
              <View
                ...
                onTouchEnd={() => setShowSelectTime(false)} // This will close the modal when tapping outside
              >
                <TouchableWithoutFeedback>
                  <View
                    className="w-full h-[33%] rounded-t-xl pt-2 bg-white dark:bg-black/90"
                  ...
                  >
                    <View>
                      <View className="mx-auto mt-4">{renderTimePicker()}</View>
                      <TouchableHighlight
                        className="absolute left-2 border rounded-lg border-red-500 px-2 py-1"
                        underlayColor={"transparent"}
                        onPress={() => {
                          setShowSelectTime(false);
                          setTimeSelect(dayjs(new Date()).format("hh:mm A"));
                        }}
                      >
                        <Text className="text-base font-semibold text-red-500">
                          Cancel
